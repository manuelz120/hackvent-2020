...
while (1) {
    ...
    last if ( $k =~ /q/ );
    $k = substr( $k, 2, 1 );
    $dx = ( $k eq 'C' ) - ( $k eq 'D' ); # left or right
    ...
    # select( undef, undef, undef, 0.28 ); # Sleep
    if ( cb( $__, $bx, ++$by ) ) {
        last
          if ( $by < 2 );
        # _s( $__, $_b, $bx, $by - 1 ); # contains the collition detection logic
        n();
    }
    else { 
        # This seems the old path
        bl( $__, " ", 0, $bx, $by - 1 ); 
    }
}
sleep(1);
